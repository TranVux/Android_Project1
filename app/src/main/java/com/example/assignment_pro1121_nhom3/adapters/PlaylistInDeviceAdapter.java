...
public class PlaylistInDeviceAdapter extends RecyclerView.Adapter<PlaylistInDeviceAdapter.MyPlaylistViewHolder> {


    private Context context;
    private List<Music> list;
    public PlaylistInDeviceAdapter.ItemChartEvent itemChartEvent;

    public PlaylistInDeviceAdapter(Context context, PlaylistInDeviceAdapter.ItemChartEvent itemChartEvent) {
    ...
    public MyPlaylistViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_playlist_device, parent, false);
    ...
    public void onBindViewHolder(@NonNull MyPlaylistViewHolder holder, int position) {
        ...
        Glide.with(context).load(list.get(position).getThumbnailUrl()).diskCacheStrategy(DiskCacheStrategy.DATA).into(holder.imgSong1);
